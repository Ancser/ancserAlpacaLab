...
# --- Page: Backtest ---
    ...
    bt_tab1, bt_tab2 = st.tabs(["🧬 Factor Model", "⚡ Unicorn Edge"])

    # ================================================================
    # TAB 2: UNICORN EDGE
    # ================================================================
    with bt_tab2:
        st.subheader("Unicorn Edge Strategy")
        st.caption("Value + Short-term Reversal, only active in drift (trending) regime. Walk-forward backtest with frozen risk scaling.")

        # ── Parameters ──────────────────────────────────────────────
        ue_col1, ue_col2 = st.columns(2)
        with ue_col1:
            st.markdown("**Signal Parameters**")
            ue_alpha = st.slider("Value Weight (α)", 0.0, 1.0, 0.7, 0.05,
                                 help="α=1 → pure value (inverse price rank). α=0 → pure reversal.")
            ue_rev_window = st.slider("Reversal Window (days)", 5, 30, 10, 1,
                                      help="Trailing window for short-term reversal signal.")
            ue_drift_window = st.slider("Drift Window (days)", 21, 126, 63, 1,
                                        help="Rolling window to measure % of up-days.")
            ue_drift_threshold = st.slider("Drift Threshold", 0.50, 0.80, 0.60, 0.01,
                                           help="Min fraction of up-days to classify as drift regime.")
            ue_tc_bps = st.slider("Transaction Cost (bps)", 0.0, 5.0, 0.6, 0.1)

        with ue_col2:
            st.markdown("**Walk-Forward Period Split**")
            ue_current_year = datetime.today().year
            ue_train_start = st.selectbox("Train Start", range(2015, ue_current_year), index=0, key="ue_train_start")
            ue_train_end = st.selectbox("Train End", range(ue_train_start + 1, ue_current_year), index=4, key="ue_train_end")
            ue_test_start = st.selectbox("Test Start", range(ue_train_end, ue_current_year), index=0, key="ue_test_start")
            ue_test_end_year = st.selectbox("Test End", range(ue_test_start, ue_current_year + 1),
                                            index=ue_current_year - ue_test_start, key="ue_test_end")
            st.caption(f"Train: {ue_train_start}–{ue_train_end} | Test: {ue_test_start}–{ue_test_end_year}")

            st.markdown("**Universe & Data**")
            try:
                from ancser_quant.data.constituents import TICKERS as _ALL_TICKERS
            except ImportError:
                _ALL_TICKERS = []
            _tech10 = "AAPL, MSFT, GOOGL, AMZN, NVDA, TSLA, META, AMD, SPY, QQQ"
            _full_univ = ", ".join(_ALL_TICKERS) if _ALL_TICKERS else _tech10
            ue_u_preset = st.radio("Universe Preset", ["Tech 10", "S&P+Nasdaq"], index=1,
                                   horizontal=True, key="ue_u_preset")
            ue_universe_input = st.text_area("Symbols", value=_full_univ if ue_u_preset == "S&P+Nasdaq" else _tech10,
                                             height=60, label_visibility="collapsed", key="ue_universe_input")
            ue_universe = [s.strip().upper() for s in ue_universe_input.split(',') if s.strip()]
            st.caption(f"{len(ue_universe)} symbols")

            _has_keys = os.getenv("APCA_API_KEY_ID") and os.getenv("APCA_API_SECRET_KEY")
            ue_data_source = st.radio("Data Source", ["Yahoo (2015+)", "Alpaca (2021+)"],
                                      index=1 if _has_keys else 0, key="ue_data_source")

        st.markdown("---")
        ue_run_col, ue_apply_col = st.columns(2)
        ue_run_bt = ue_run_col.button("▶️ Run Unicorn Edge Backtest", type="primary", key="ue_run")

        # ── Backtest Execution ───────────────────────────────────────
        if ue_run_bt:
            with st.spinner("Fetching data and running walk-forward backtest..."):
                try:
                    from ancser_quant.alpha.unicorn_edge import run_backtest as ue_run_backtest
                    from ancser_quant.data.yahoo_adapter import YahooAdapter

                    # Fetch price data
                    ue_fetch_start = f"{ue_train_start}-01-01"
                    ue_fetch_end = f"{ue_test_end_year}-12-31" if ue_test_end_year < ue_current_year else datetime.today().strftime('%Y-%m-%d')

                    if "Alpaca" in ue_data_source:
                        from ancser_quant.data.alpaca_adapter import AlpacaAdapter
                        _ue_adapter = AlpacaAdapter()
                    else:
                        _ue_adapter = YahooAdapter()

                    ue_raw = _ue_adapter.fetch_history(ue_universe, ue_fetch_start, ue_fetch_end).collect().to_pandas()

                    if ue_raw.empty:
                        st.error("No data fetched. Try Yahoo source or a smaller universe.")
                    else:
                        ue_raw['timestamp'] = pd.to_datetime(ue_raw['timestamp']).dt.tz_localize(None).dt.normalize()
                        ue_prices = ue_raw.pivot(index='timestamp', columns='symbol', values='close').ffill().bfill()

                        ue_result = ue_run_backtest(
                            prices=ue_prices,
                            train_start=f"{ue_train_start}-01-01",
                            train_end=f"{ue_train_end}-12-31",
                            test_start=f"{ue_test_start}-01-01",
                            test_end=ue_fetch_end,
                            transaction_cost_bps=ue_tc_bps,
                            alpha=ue_alpha,
                            reversal_window=ue_rev_window,
                            drift_window=ue_drift_window,
                            drift_threshold=ue_drift_threshold,
                        )

                        st.session_state['ue_result'] = ue_result
                        st.session_state['ue_scale_factor'] = ue_result['scale_factor']
                        st.success("Backtest completed!")

                except Exception as _ue_e:
                    st.error(f"Backtest failed: {_ue_e}")
                    import traceback
                    st.code(traceback.format_exc())

        # ── Results display ──────────────────────────────────────────
        if 'ue_result' in st.session_state:
            ue_res = st.session_state['ue_result']
            ue_eq = (1 + ue_res['returns']).cumprod() if not ue_res['returns'].empty else pd.Series([1.0])

            r1, r2, r3, r4, r5, r6 = st.columns(6)
            r1.metric("Ann. Return", f"{ue_res['ann_return']:.2%}")
            r2.metric("Sharpe", f"{ue_res['sharpe']:.2f}")
            r3.metric("Ann. Vol", f"{ue_res['ann_vol']:.2%}")
            r4.metric("Max DD", f"{ue_res['max_drawdown']:.2%}")
            r5.metric("Scale Factor", f"{ue_res['scale_factor']:.2f}x")
            r6.metric("Train Sharpe", f"{ue_res['train_sharpe']:.2f}")

            if not ue_res['kill_switch_active']:
                st.warning("Kill switch triggered during test period — strategy halted early.")

            # Equity curve
            fig_ue = go.Figure()
            fig_ue.add_trace(go.Scatter(
                x=ue_eq.index, y=ue_eq.values,
                mode='lines', name='Unicorn Edge (Test)',
                line=dict(color='#AB63FA', width=2),
                fill='tozeroy', fillcolor='rgba(171,99,250,0.1)'
            ))
            fig_ue.update_layout(
                title="Unicorn Edge — Test Period Cumulative Return",
        ...
        # ── Apply to Trade ───────────────────────────────────────────
        st.markdown("**Apply Unicorn Edge to Live Trading**")
        if st.button("💾 Apply Unicorn Edge to Trade", type="primary", key="ue_apply"):
            import json
            _scale = st.session_state.get('ue_scale_factor', 1.0)
            _ue_live_config = {
                "strategy_type": "unicorn_edge",
                "ue_alpha": ue_alpha,
                "ue_reversal_window": ue_rev_window,
                "ue_drift_window": ue_drift_window,
                "ue_drift_threshold": ue_drift_threshold,
                "ue_scale_factor": round(_scale, 4),
                "universe": ue_universe,
                "leverage": 1.0,
                "last_updated": datetime.now().isoformat()
            }
            try:
                os.makedirs("config", exist_ok=True)
                with open("config/live_strategy.json", 'w') as _f:
                    json.dump(_ue_live_config, _f, indent=4)
                st.success("Unicorn Edge config saved. Use 'Force Execute Now' on the Factor Model tab to apply.")
                st.json(_ue_live_config)
            except Exception as _e:
                st.error(f"Failed to save: {_e}")

        if 'ue_scale_factor' not in st.session_state:
            st.caption("Run a backtest first to auto-fill the scale factor before applying.")

    # ================================================================
    # TAB 1: FACTOR MODEL (existing content)
    # ================================================================
    with bt_tab1:
